...
class UpsampleCBR(torch.nn.Sequential):
    def __init__(self, input_channels, output_channels, count=1, num_flat=0):
        ...
        super(UpsampleCBR, self).__init__(*layers)o
...
class CmapPafHeadAttention(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        xc = self.cmap_up(x)
        print("Shape after cmap_up:", xc.shape)
        ac = torch.sigmoid(self.cmap_att(xc))
        print("Shape after cmap_att:", ac.shape)
        print("Tensor before multiplying with ac:", xc)
        print("ac tensor:", ac)

        xp = self.paf_up(x)
        print("Shape after paf_up:", xp.shape)
        ap = torch.tanh(self.paf_att(xp))
        print("Shape after paf_att:", ap.shape)
        print("Tensor before multiplying with ap:", xp)
        print("ap tensor:", ap)

        cmap_output = self.cmap_conv(xc * ac)
        paf_output = self.paf_conv(xp * ap)
        
        print("Shape of cmap_output:", cmap_output.shape)
        print("Shape of paf_output:", paf_output.shape)
        
        return cmap_output, paf_output
