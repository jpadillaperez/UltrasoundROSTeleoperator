...
#else
  progSocket->Send(str.Data);
  std::cout <<"SendToURClient:" <<str.Data << std::endl;
...
{
  pose.Translation()[0] = ur_interface.toolPose[0];
  pose.Translation()[1] = ur_interface.toolPose[1];
  pose.Translation()[2] = ur_interface.toolPose[2];

  vct3 axis(ur_interface.toolPose[3],
            ur_interface.toolPose[4],
            ur_interface.toolPose[5]);
...
void RobotServer::SendDoubleVec(const unsigned long mode, const vctDoubleVec& v, const bool pad)
{
  ...
  sprintf(JointPosCmdString,
            ...
            v[0], v[1], v[2], v[3], v[4], v[5], 1.0, 0.2);
  urSocket.Send(JointPosCmdString);
...
bool RobotServer::ConnectToUR(const std::string& ur_ip, unsigned short ur_port)
{
  ...
  std::cout << "IP: " << ur_ip.c_str() <<  ":" <<ur_port<< std::endl;
...
void RobotServer::GetJointModes(vctInt6 &modes) const
{
  ...
  {
    modes[i] = ur_interface.jointModes[i];
  }
}
...
void RobotServer::IsJointHomed(vctBool6 &stateVec) const
{
  ...
  for (unsigned long i = 0; i < 6; ++i)
  {
    stateVec[i] = static_cast<UniversalRobot::JointMode>(ur_interface.jointModes[i]) == UniversalRobot::JOINT_INITIALIZATION;
  }
}
...
void RobotServer::IsHomed(bool &state) const
{
  ...
  {
    homed = homed && (static_cast<UniversalRobot::JointMode>(ur_interface.jointModes[i]) == UniversalRobot::JOINT_INITIALIZATION);
  }
...
{
  state = static_cast<UniversalRobot::RobotMode>(ur_interface.robotMode_) == UniversalRobot::ROBOT_READY;
}

void RobotServer::IsEStop(bool &state) const
{
  state = static_cast<UniversalRobot::RobotMode>(ur_interface.robotMode_) == UniversalRobot::ROBOT_EMERGENCY_STOPPED;
}
...
{
  state = static_cast<UniversalRobot::RobotMode>(ur_interface.robotMode_) == UniversalRobot::ROBOT_SERCURITY_STOPPED;
}
