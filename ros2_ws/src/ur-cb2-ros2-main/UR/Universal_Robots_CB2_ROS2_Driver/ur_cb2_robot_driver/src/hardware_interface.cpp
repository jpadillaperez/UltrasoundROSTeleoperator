...
std::vector<hardware_interface::StateInterface> URPositionHardwareInterface::export_state_interfaces()
{
  ...
  state_interfaces.emplace_back(
      hardware_interface::StateInterface("speed_scaling", "speed_scaling_factor", &speed_scaling_factor_));
...
std::vector<hardware_interface::CommandInterface> URPositionHardwareInterface::export_command_interfaces()
{
  ...
  command_interfaces.emplace_back(hardware_interface::CommandInterface(
      "speed_scaling", "target_speed_fraction_async_success", &target_speed_fraction_async_success_));
...
hardware_interface::return_type URPositionHardwareInterface::read(const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  for (uint i = 0; i < info_.joints.size(); i++) {
    ur_joint_positions_[i] = ur_interface.jsPosition[i];
    ur_joint_velocities_[i] = ur_interface.jsVelocity[i];
    ur_joint_efforts_[i] = ur_interface.jsEffort[i];
...
hardware_interface::return_type URPositionHardwareInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (isConnectedToProg && robot_mode_ == UniversalRobot::ROBOT_RUNNING && packet_read_ && time_since_last_send_ >= rclcpp::Duration(0, 8000000))
  {
