...
class TRTPose(Node):
    def __init__(self):
        ...
        self.depth_subscriber = self.create_subscription(ImageMsg, '/depth_to_rgb/image_raw', self.depth_callback, 10)
        ...
        self.depth_image_with_keypoints_pub = self.create_publisher(ImageMsg, 'depth/image_with_keypoints', 10)
        # Publisher for Body Joints and Skeleton
        self.body_joints_pub = self.create_publisher(Marker, 'body_joints', 1000000)
        self.body_skeleton_pub = self.create_publisher(Marker, 'body_skeleton', 1000000)
        # Publishing pose Message
        self.publish_pose = self.create_publisher(PersonDetection, 'pose_msgs', 1000000)
        # Transform Images
    ...
    def depth_callback(self, msg):
        try:
            ...
            # Convert depth image to a color image for visualization
            depth_color_image = cv2.applyColorMap(cv2.convertScaleAbs(self.depth_image, alpha=0.03), cv2.COLORMAP_JET)

            # Overlay 2D keypoints on the depth image and convert to 3D
            if self.annotated_image is not None and self.counts is not None:
                keypoints_2d = []
                for i in range(int(self.counts[0])):
                    for k in range(18):
                        _idx = self.objects[0, i, k]
                        if _idx >= 0:
                            _location = self.peaks[0, k, _idx, :].numpy()
                            u = int(_location[1] * self.depth_image.shape[1])
                            v = int(_location[0] * self.depth_image.shape[0])
                            keypoints_2d.append((u, v))

                            self.get_logger().info(f'Keypoint {k} at (u={u}, v={v})')

                            if 0 <= u < self.depth_image.shape[1] and 0 <= v < self.depth_image.shape[0]:
                                cv2.circle(depth_color_image, (u, v), 5, (0, 255, 0), -1)

                # Convert 2D keypoints to 3D
                keypoints_3d = self.convert_to_3d(keypoints_2d)
                #for kpt in keypoints_3d:
                    # self.get_logger().info(f'3D Keypoint: ({kpt[0]:.2f}, {kpt[1]:.2f}, {kpt[2]:.2f})')

            # Publish the depth image with keypoints
            depth_image_with_keypoints_msg = self.bridge.cv2_to_imgmsg(depth_color_image, encoding='bgr8')
            self.depth_image_with_keypoints_pub.publish(depth_image_with_keypoints_msg)
        except Exception as e:
    ...
    def write_body_part_msg(self, pixel_location):
        ...
        body_part_pixel_loc.z = float(depth_value_mm / 10.0)
        self.get_logger().info(f'3D keypoint: ({body_part_pixel_loc.x:.2f}, {body_part_pixel_loc.y:.2f}, {body_part_pixel_loc.z:.2f})')
    ...
    def add_point_to_marker(self, body_part_msg):
        ...
        p.z = float((body_part_msg.z / self.height) * self.point_range)
        return p
